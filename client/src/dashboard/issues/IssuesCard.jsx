...
export default function IssueCard({ issue, handler, loading }) {
    ...
    return (
        <div className='bg-white min-w-[300px]  rounded-md shadow-md dark:bg-gray-800 border border-gray-300 dark:border-gray-700'>
            ...
            <div className="p-3 ">
                <div className="text-xs flex items-center border-b pb-1 justify-between">
                    ...
                    {currUser?.role === 'admin' && <div>
                        <p>By : {issue?.reportedBy?.name}</p>
                        <p> Roll: {issue?.reportedBy?.rollNumber}</p>
                    </div>}
                </div>

                <p className='text-gray-600 my-2 dark:text-gray-400'>{issue?.issueDesc}</p>
