...
const ForgotPass = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true);
        try {
            const res = await axiosInstance.post('/user/forgotpass', {
                ...data
            })
                    ...
                    <h1 className='text-center text-2xl font-bold text-gray-800 dark:text-gray-100 my-4'>Forget Password</h1>
{
             !linkSent?       <form className="w-full" onSubmit={(e) => handleSubmit(e)}>
                     ...
                     <input className='w-full my-2 p-2 px-3  outline-none rounded-md bg-gray-50 dark:bg-stone-800 dark:text-gray-50 '
                        ...
                        <p className='text-center mt-8 gap-1 flex items-center justify-center text-xs dark:text-gray-100'> Having Account? <Link to='/login' className='text-blue-800 font-semibold'>Login here</Link></p>
                    </form>
